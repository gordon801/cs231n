...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # Embed caption: (N, T) -> (N, T, W)
        embedded_captions = self.embedding(captions)

        # Add positional encoding: (N, T, W)
        pe_captions = self.positional_encoding(embedded_captions)

        # Project image features into the same dimensions: 
        # (N,D) -projection-> (N,W) -unsqueeze-> (N,1,W) -repeat-> (N,T,W)
        img_features = self.visual_projection(features).unsqueeze(1)
        img_features = img_features.repeat(1, T, 1)

        # Prepare a mask (T, T) for masking out future timesteps in captions.
        tgt_mask = torch.tril(torch.ones(T,T))

        # Apply the transformer decoder to the text & image embeddings, along with the
        # tgt_mask.
        output = self.transformer(tgt=pe_captions, memory=img_features, tgt_mask=tgt_mask)
        
        # Project output to scores per token.
