...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    # Forward pass: h_t = tanh(h_t-1 * W_hh + x_t * W_xh)
...
def rnn_step_backward(dnext_h, cache):
    ...
    # d = c + beta
    # e = tanh(d)
    # h_t / f = tanh(d) = tanh(h_t-1 * W_hh + x_t * W_xh)
    
    # dd = df/dd = df/de * de/dd = dnext_h * (1 - tanh**2(d))
    # dbeta = df/dbeta = df/dd * dd/dbeta = dd * 1 (NxH -> H, sum(axis=0))
    # dc = df/dc = df/dd * dd/dc = dd * 1
    # da = df/da = df/dc * dc/da = dc * 1
    # db = df/db = df/dc * dc/db = dc * 1
    # dh_t-1 = df/dh_t-1 = df/da * da/dh_t-1 = da * W_h
    # dw_h = df/dw_h = df/da * da/dw_h = da * h_t-1
    # dx = df/dh_t-1 = df/db * db/dx = db * W_x
    # dw_x = df/dh_t-1 = df/db * db/dw_x = db * x
    ...
    # dh_t-1 = da * W_h = dd * W_h
    dprev_h = np.dot(dd, Wh.T)
    
    # dWx = db * x = dd * x
    dWx = np.dot(x.T, dd)
    
    # dWh = da * h_t-1 = dd * prev_h
    dWh = np.dot(prev_h.T, dd)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    N, T, D = x.shape
    H = b.shape[0]
    h = np.zeros((N, T+1, H)) # Initialise shape with T+1 to include the initial hidden state h0
    h[:,0,:] = h0 # Set initial hidden state
    cache = {}
    
    for t in range(T):
        # Forward pass: h_t = tanh(h_t-1 * W_hh + x_t * W_xh), for t = [1, ..., T]
        h[:, t+1, :], cache[t] = rnn_step_forward(x[:, t, :], h[:,t,:], Wx, Wh, b)
    h = h[:,1:,:] # Remove initial hidden state
...
def rnn_backward(dh, cache):
    ...
    for t in range(T)[::-1]:
        # Compute backward pass using rnn_step_backward at each t [T, T-1, ..., 1]
        dh_ag = dh[:, t, :] + dprev_h # Add gradient from previous timestep to the upstream gradient at current t
        dx[:, t, :], dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dh_ag, cache[t])
        
        # Gradient of loss wrt to weights and bias = sum(gradient of loss at timestep t)
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
    
    # Last computed prev_h = gradient of loss wrt to initial hidden state = dh0
    dh0 = dprev_h 
...
def word_embedding_forward(x, W):
    ...
    # Use array indexing to create word embeddings array
    out[:,:,:] = W[x[:,:], :]
    
    # Store values for backward pass in cache
    cache = (x, W)
...
def word_embedding_backward(dout, cache):
    ...
    # Backward pass: Add gradient from dout to dW at indices specified by x. 
    np.add.at(dW, x, dout)
